 ...
 *  limitations under the License.
 */
var keyvalue_option_validation, siunitx_options_definition, unit_definitions, unit_parser, number_parser_regex, siunitx_commands;
...
number_parser_regex = function (SIunitxOptions) {
  var exports = {};
  var TEX = MathJax.InputJax.TeX;
  var SINumberParser = exports.SINumberParser = MathJax.Object.Subclass({
    ...
    GenerateRegex: function (options) {
      ...
      var decimal_number = '(' + digit + '*)(?:' + decimal_sep + '(' + digit + '*))?';
      // 2 grps
      var imaginary_number = '(?:' + decimal_number + complex_root + '|' + complex_root + decimal_number + ')';
      // 2 + 2 = 4 grps
      var complex_number = sign + '?' + decimal_number + '(?:' + sign + imaginary_number + ')?';
      // 1 + 2 + 1 + 4 = 8 grps
      var full_number = complex_number + exponent + '?';
      // 8+1= 9 grps
      var multipart = '(' + product + '|' + quotient + ')';
      // 1 grp
      var multi_part_number = full_number + '(?:' + multipart + '(' + full_number + '(?:' + multipart + full_number + ')*))?';
      // 9+1+1+9+1+9 = 30 grps
...
}(siunitx_options_definition, unit_definitions, unit_parser, number_parser_regex);
}());
  // amd-replace-stop
});
